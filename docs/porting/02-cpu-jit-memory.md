...
- **`temporaryGPR[4]` is referenced only by `BackendX64.cpp:835-893`.** Safe to delete with the purge. `PPCInterpreter_t` is never serialized (only `malloc`+`memset`, `PPCInterpreterMain.cpp:20-21`) so the layout shift is contained by the 13 `static_assert`s at `BackendAArch64.cpp:366-378`.
