...
`LatteTextureReadbackMtl.cpp:30-32`: `RequestSoonCommit()` is commented out and the code calls `CommitCommandBuffer()` unconditionally, forcing a full flush on every readback. Switch to `RequestSoonCommit()` (`MetalRenderer.h:297-300`, sets the threshold to `drawcalls + 8`), which lets a few more draws batch in before the flush. `ForceFinish()`'s `waitUntilCompleted()` (`:46`) is unavoidable — readback is inherently synchronous in Cemu's design. Also uncomment the `hasReadback` term in `draw_endSequence` (`MetalRenderer.cpp:1479`) once this is in.
