...
~~**There is currently zero QoS anywhere** — no `pthread_set_qos_class_self_np`, no `QOS_CLASS_*`.~~ **Superseded — QoS is implemented.** `SetThreadName(name, ThreadRole)` applies it via `pthread_set_qos_class_self_np` (`util/helpers/helpers.cpp:121-179`). The analysis below is retained because it is why the ordering worked out; read it as rationale, not as current state.
...
~~**`src/util/helpers/fspinlock.h` is a pure spin loop** (`m_lockBool.exchange` + `_mm_pause()` — which on arm64 is the `yield` shim from `precompiled.h:358`).~~ **Fixed — it is `os_unfair_lock` now.** The rest of this section explains why that had to happen before QoS, which is the order it happened in. `s_ppcRecompilerState.recompilerSpinlock` is taken by:
...
| **R3** ~~QoS-induced hang~~ **DISCHARGED** | ~~**QoS-induced hang** via `FSpinlock` inversion between a `UTILITY` recompiler and `USER_INTERACTIVE` guest cores. Presents as a total freeze, not a slowdown.~~ `FSpinlock` became `os_unfair_lock` **before** QoS was assigned, which is the mitigation this row prescribes. | ~~`FSpinlock` is a pure spin with no donation~~; `recompilerSpinlock` is genuinely shared across those tiers, which is why the ordering mattered. | **Do §4.3 before §4.2.** Then: run with `taskpolicy -b` (force background) on the recompiler thread and hammer code invalidation. Also `sample $(pgrep Cemu)` during any freeze — a stack pegged in `FSpinlock::lock` is diagnostic. |
